---@meta
-- Generated LuaLS annotations for SpellFlyout (Build 12.1.5.69594 [v1215]). Do not edit.

---Row representation of SpellFlyout in build 12.1.5.69594.
---@class dbc.row.v1215.SpellFlyout : RowProxy
---@field ID integer
---@field Name_lang string
---@field Description_lang string
---@field Flags integer
---@field ClassMask integer
---@field SpellIconFileID integer
---@field RaceMasks integer[]

local row = {}

---Gets the value of field 'ID'.
---@return integer value
function row:GetID() end

---Sets the value of field 'ID'.
---@param value integer
---@return dbc.row.v1215.SpellFlyout self
function row:SetID(value) end

---Gets the value of field 'Name_lang'.
---@return string value
function row:GetName_lang() end

---Sets the value of field 'Name_lang'.
---@param value string
---@return dbc.row.v1215.SpellFlyout self
function row:SetName_lang(value) end

---Gets the value of field 'Description_lang'.
---@return string value
function row:GetDescription_lang() end

---Sets the value of field 'Description_lang'.
---@param value string
---@return dbc.row.v1215.SpellFlyout self
function row:SetDescription_lang(value) end

---Gets the value of field 'Flags'.
---@return integer value
function row:GetFlags() end

---Sets the value of field 'Flags'.
---@param value integer
---@return dbc.row.v1215.SpellFlyout self
function row:SetFlags(value) end

---Gets the value of field 'ClassMask'.
---@return integer value
function row:GetClassMask() end

---Sets the value of field 'ClassMask'.
---@param value integer
---@return dbc.row.v1215.SpellFlyout self
function row:SetClassMask(value) end

---Gets the value of field 'SpellIconFileID'.
---@return integer value
function row:GetSpellIconFileID() end

---Sets the value of field 'SpellIconFileID'.
---@param value integer
---@return dbc.row.v1215.SpellFlyout self
function row:SetSpellIconFileID(value) end

---Navigates foreign relationship to 'FileData' via 'SpellIconFileID'.
---@return dbc.row.v1215.FileData|nil
function row:GetSpellIconFile() end

---Creates a related 'FileData' row and automatically links 'SpellIconFileID'.
---@param data? table
---@return dbc.row.v1215.FileData
function row:CreateRelated(data) end

---Gets the value of field 'RaceMasks'.
---@return integer[] value
function row:GetRaceMasks() end

---Sets the value of field 'RaceMasks'.
---@param value integer[]
---@return dbc.row.v1215.SpellFlyout self
function row:SetRaceMasks(value) end

---Gets element at 1-based index in 'RaceMasks'.
---@param index integer
---@return integer value
function row:GetRaceMasksItem(index) end

---Table container for SpellFlyout (Build 12.1.5.69594).
---@class dbc.Table.v1215.SpellFlyout : DbcTable
---@field [integer] dbc.row.v1215.SpellFlyout
local tbl = {}

---Creates and appends a new row to the table.
---@param id? integer Primary key
---@param data? table Optional initial values
---@return dbc.row.v1215.SpellFlyout
function tbl:Create(id, data) end

---Finds a row by primary key (nil if not found).
---@param id integer
---@return dbc.row.v1215.SpellFlyout|nil
function tbl:FindById(id) end

---Gets a row by primary key (throws if not found).
---@param id integer
---@return dbc.row.v1215.SpellFlyout
function tbl:GetRowById(id) end

---Creates a LINQ query over rows in this table.
---@return dbc.Query<dbc.row.v1215.SpellFlyout>
function tbl:Query() end

---Returns all rows related to a foreign ID via the relationship block.
---@param foreign_id integer
---@return dbc.row.v1215.SpellFlyout[]
function tbl:GetByRelation(foreign_id) end

return {}
