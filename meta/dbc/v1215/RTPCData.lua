---@meta
-- Generated LuaLS annotations for RTPCData (Build 12.1.5.69594 [v1215]). Do not edit.

---Row representation of RTPCData in build 12.1.5.69594.
---@class dbc.row.v1215.RTPCData : RowProxy
---@field ID integer
---@field RTPCID integer
---@field Field_9_0_1_33978_001 integer
---@field CreatureID integer
---@field SpellID integer
---@field Field_9_0_1_33978_004 integer

local row = {}

---Gets the value of field 'ID'.
---@return integer value
function row:GetID() end

---Sets the value of field 'ID'.
---@param value integer
---@return dbc.row.v1215.RTPCData self
function row:SetID(value) end

---Gets the value of field 'RTPCID'.
---@return integer value
function row:GetRTPCID() end

---Sets the value of field 'RTPCID'.
---@param value integer
---@return dbc.row.v1215.RTPCData self
function row:SetRTPCID(value) end

---Navigates foreign relationship to 'RTPC' via 'RTPCID'.
---@return dbc.row.v1215.RTPC|nil
function row:GetRTPC() end

---Creates a related 'RTPC' row and automatically links 'RTPCID'.
---@param data? table
---@return dbc.row.v1215.RTPC
function row:CreateRelated(data) end

---Gets the value of field 'Field_9_0_1_33978_001'.
---@return integer value
function row:GetField_9_0_1_33978_001() end

---Sets the value of field 'Field_9_0_1_33978_001'.
---@param value integer
---@return dbc.row.v1215.RTPCData self
function row:SetField_9_0_1_33978_001(value) end

---Gets the value of field 'CreatureID'.
---@return integer value
function row:GetCreatureID() end

---Sets the value of field 'CreatureID'.
---@param value integer
---@return dbc.row.v1215.RTPCData self
function row:SetCreatureID(value) end

---Navigates foreign relationship to 'Creature' via 'CreatureID'.
---@return dbc.row.v1215.Creature|nil
function row:GetCreature() end

---Creates a related 'Creature' row and automatically links 'CreatureID'.
---@param data? table
---@return dbc.row.v1215.Creature
function row:CreateRelated(data) end

---Gets the value of field 'SpellID'.
---@return integer value
function row:GetSpellID() end

---Sets the value of field 'SpellID'.
---@param value integer
---@return dbc.row.v1215.RTPCData self
function row:SetSpellID(value) end

---Navigates foreign relationship to 'Spell' via 'SpellID'.
---@return dbc.row.v1215.Spell|nil
function row:GetSpell() end

---Creates a related 'Spell' row and automatically links 'SpellID'.
---@param data? table
---@return dbc.row.v1215.Spell
function row:CreateRelated(data) end

---Gets the value of field 'Field_9_0_1_33978_004'.
---@return integer value
function row:GetField_9_0_1_33978_004() end

---Sets the value of field 'Field_9_0_1_33978_004'.
---@param value integer
---@return dbc.row.v1215.RTPCData self
function row:SetField_9_0_1_33978_004(value) end

---Table container for RTPCData (Build 12.1.5.69594).
---@class dbc.Table.v1215.RTPCData : DbcTable
---@field [integer] dbc.row.v1215.RTPCData
local tbl = {}

---Creates and appends a new row to the table.
---@param id? integer Primary key
---@param data? table Optional initial values
---@return dbc.row.v1215.RTPCData
function tbl:Create(id, data) end

---Finds a row by primary key (nil if not found).
---@param id integer
---@return dbc.row.v1215.RTPCData|nil
function tbl:FindById(id) end

---Gets a row by primary key (throws if not found).
---@param id integer
---@return dbc.row.v1215.RTPCData
function tbl:GetRowById(id) end

---Creates a LINQ query over rows in this table.
---@return dbc.Query<dbc.row.v1215.RTPCData>
function tbl:Query() end

---Returns all rows related to a foreign ID via the relationship block.
---@param foreign_id integer
---@return dbc.row.v1215.RTPCData[]
function tbl:GetByRelation(foreign_id) end

return {}
