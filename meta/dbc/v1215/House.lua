---@meta
-- Generated LuaLS annotations for House (Build 12.1.5.69594 [v1215]). Do not edit.

---Row representation of House in build 12.1.5.69594.
---@class dbc.row.v1215.House : RowProxy
---@field ID integer
---@field Name string
---@field Field_12_0_0_63534_001 integer
---@field Field_12_0_0_63534_002 integer
---@field Field_12_0_0_63534_003 integer

local row = {}

---Gets the value of field 'ID'.
---@return integer value
function row:GetID() end

---Sets the value of field 'ID'.
---@param value integer
---@return dbc.row.v1215.House self
function row:SetID(value) end

---Gets the value of field 'Name'.
---@return string value
function row:GetName() end

---Sets the value of field 'Name'.
---@param value string
---@return dbc.row.v1215.House self
function row:SetName(value) end

---Gets the value of field 'Field_12_0_0_63534_001'.
---@return integer value
function row:GetField_12_0_0_63534_001() end

---Sets the value of field 'Field_12_0_0_63534_001'.
---@param value integer
---@return dbc.row.v1215.House self
function row:SetField_12_0_0_63534_001(value) end

---Gets the value of field 'Field_12_0_0_63534_002'.
---@return integer value
function row:GetField_12_0_0_63534_002() end

---Sets the value of field 'Field_12_0_0_63534_002'.
---@param value integer
---@return dbc.row.v1215.House self
function row:SetField_12_0_0_63534_002(value) end

---Gets the value of field 'Field_12_0_0_63534_003'.
---@return integer value
function row:GetField_12_0_0_63534_003() end

---Sets the value of field 'Field_12_0_0_63534_003'.
---@param value integer
---@return dbc.row.v1215.House self
function row:SetField_12_0_0_63534_003(value) end

---Table container for House (Build 12.1.5.69594).
---@class dbc.Table.v1215.House : DbcTable
---@field [integer] dbc.row.v1215.House
local tbl = {}

---Creates and appends a new row to the table.
---@param id? integer Primary key
---@param data? table Optional initial values
---@return dbc.row.v1215.House
function tbl:Create(id, data) end

---Finds a row by primary key (nil if not found).
---@param id integer
---@return dbc.row.v1215.House|nil
function tbl:FindById(id) end

---Gets a row by primary key (throws if not found).
---@param id integer
---@return dbc.row.v1215.House
function tbl:GetRowById(id) end

---Creates a LINQ query over rows in this table.
---@return dbc.Query<dbc.row.v1215.House>
function tbl:Query() end

---Returns all rows related to a foreign ID via the relationship block.
---@param foreign_id integer
---@return dbc.row.v1215.House[]
function tbl:GetByRelation(foreign_id) end

return {}
